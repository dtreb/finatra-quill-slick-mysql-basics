...
import com.dtreb.library.modules.DatabaseModule.DatabaseSource
import com.twitter.finagle.exp.mysql.Result
import com.twitter.util.Future
import io.getquill._

@Singleton
class QuillLibraryService @Inject() (db: DatabaseSource) extends ILibraryService {
  ...
  override def create(book: Book): Future[Result] = {
    val a = quote {
  ...
  override def update(book: Book): Future[Result] = {
    val a = quote {
  ...
  override def delete(id: Long): Future[Result] = {
    val a = quote {
      (id: Long) =>
